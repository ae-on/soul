...
def save_user(telegram_id, utm_source, full_name=None, phone=None):
    with get_db_connection() as conn:
        ...
        if cur.fetchone() is None:
            ...
            # Обновляем только utm_source, чтобы не затереть имя/телефон
            conn.execute(
...
def update_user_details(telegram_id, full_name, phone):
    """Обновляет имя и телефон существующего пользователя (с шифрованием)."""
    encrypted_name = encrypt_data(full_name)
    encrypted_phone = encrypt_data(phone)
    with get_db_connection() as conn:
        conn.execute(
            "UPDATE clients SET full_name = ?, phone_number = ? WHERE telegram_id = ?",
            (encrypted_name, encrypted_phone, telegram_id),
        )
